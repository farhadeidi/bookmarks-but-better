 ...
 * the dashboard's only path in is a daemon Vault — one source means no
 * switcher at all, and the daemon-only empty state says so rather than
 * guessing when nothing is connected.
 */
