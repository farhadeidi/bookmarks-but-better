...
pub(crate) fn is_case_sensitive(directory: &Path) -> bool {
    let lower = directory.join("bbb-case-probe");
    let upper = directory.join("BBB-CASE-PROBE");
...
pub(crate) fn folder_source(id: &str, title: Option<&str>) -> String {
    let title = title.map_or_else(String::new, |title| format!("bbb_title: {title}\n"));
    format!("---\nbbb_id: {id}\n{title}---\n")
}
