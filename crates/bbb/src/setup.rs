//! `bbb setup`: the guided first run.
//!
...
pub fn plan(prompt: &mut impl Prompt) -> Result<SetupPlan, SetupError> {
    prompt.tell("This sets up a local bbb daemon serving one vault on loopback.");
    prompt.tell("");
...
fn ask_vault_path(prompt: &mut impl Prompt) -> Result<PathBuf, SetupError> {
    loop {
        ...
        if answer.is_empty() {
            ...
            prompt.tell("A path is required — bbb never guesses which directory is yours.");
            continue;
        }
...
fn confirm_new(prompt: &mut impl Prompt, vault: &Path) -> Result<bool, SetupError> {
    if vault.join(bbb_vault_core::FOLDER_FILE_NAME).is_file() {
        prompt.tell(&format!(
    ...
    if vault.is_dir() {
        ...
        if !empty {
            prompt.tell(&format!(
                "{} already exists and is not empty. Initializing adds bbb's metadata files; nothing else is touched.",
                vault.display()
            ));
        }
    }
    ...
    if !report.is_healthy() {
        ...
        if !is_yes(&answer, false) {
            return Err(SetupError::Declined(
                "fix the vault (see `bbb doctor`) and run setup again",
            ));
        }
    }
...
mod tests {
    ...
    fn an_existing_directory_that_is_not_a_vault_still_needs_confirmation() {
        ...
        assert!(!directory.path().join(".bbb-folder.md").exists());
    }
