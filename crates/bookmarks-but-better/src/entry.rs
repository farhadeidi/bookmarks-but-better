...
//! `.bookmarks-but-better-folder.md`, which is the normal state of a folder tree the user
...
use bookmarks_but_better_vault_core::Id;
use serde::Serialize;
...
pub enum EntryRef {
    ...
    ///
    /// These are read-only: without a `.bookmarks-but-better-folder.md` there is nothing that
