...
//! Windows, via [`std::fs::File::try_lock`]) on `<vault>/.bookmarks-but-better/lock`. Advisory
...
//!
//! Both `.bookmarks-but-better` and the lock file inside it are opened through the vault's own
//! directory handle with no-follow. A `.bookmarks-but-better` that is a symbolic link or a
...
pub(crate) const STATE_DIRECTORY: &str = ".bookmarks-but-better";
...
impl VaultLock {
    ...
    /// not have to resolve `.bookmarks-but-better` a second time by name.
    ///
    /// # Errors
    ///
    ...
    /// cannot be created — including when `.bookmarks-but-better` exists but is not a real
    /// directory.
    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            Self::Held { path } => write!(
                f,
                "another bookmarks-but-better daemon already holds this vault (lock: {})",
                path.display()
            ),
...
mod tests {
    ...
    fn a_symlinked_state_directory_is_refused() {
        ...
        let error = VaultLock::acquire(&root, &vault)
