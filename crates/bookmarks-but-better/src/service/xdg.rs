...
//! is the fallback rather than the default, and why `bookmarks-but-better service status` has
...
pub(crate) const DESKTOP_FILE: &str = "bookmarks-but-better.desktop";
...
mod tests {
    ...
    fn the_entry_is_hidden_and_binds_loopback() {
        let spec =
            ServiceSpec::unchecked("/usr/local/bin/bookmarks-but-better", "/home/user/Vault");
        let text = desktop_entry(&spec);
