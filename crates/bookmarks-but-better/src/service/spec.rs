...
impl ServiceSpec {
    ...
    /// is the same refusal `bookmarks-but-better serve` makes, repeated here because a service
...
mod tests {
    ...
    fn a_relative_path_is_refused_for_the_vault_and_the_executable() {
        assert!(matches!(
            ServiceSpec::new(
                ...
                absolute("/home/user/Vault", r"C:\Users\user\Vault")
            ),
            Err(SpecError::RelativePath {
                what: "executable",
                ..
        ...
        assert!(matches!(
            ServiceSpec::new(
                absolute(
                    "/usr/local/bin/bookmarks-but-better",
                    r"C:\Program Files\bookmarks-but-better\bookmarks-but-better.exe"
                ),
                "Vault"
            ),
    ...
    fn a_ui_directory_is_appended_only_when_set() {
        ...
        let ui_dir = absolute(
            "/opt/bookmarks-but-better/ui",
            r"C:\Program Files\bookmarks-but-better\ui",
        );
