...
//!   `bookmarks-but-better service start` that printed "started" for one would be reporting a
...
impl ServiceLayout {
    ...
    pub fn definition_path(&self, kind: ServiceKind) -> PathBuf {
        match kind {
            ...
            ServiceKind::ScheduledTask => self
                .config_home
...
/// What `bookmarks-but-better service status` found.
#[derive(Debug, Clone, PartialEq, Eq)]
    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::Unwired { kind, instead } => write!(
                f,
                "installing a {} is supported, but starting and stopping one from `bookmarks-but-better` is not implemented yet; {instead}",
                kind.describe()
            ),
...
mod tests {
    ...
    fn uninstall_removes_the_definition_and_never_the_vault() {
        for &kind in KINDS {
            ...
            let spec = ServiceSpec::unchecked(
    ...
    fn definitions_land_in_the_conventional_place_for_each_platform() {
        ...
        assert_eq!(
            ...
            home.join(".config/systemd/user/bookmarks-but-better.service")
        );
        assert_eq!(
            layout.definition_path(ServiceKind::XdgAutostart),
            home.join(".config/autostart/bookmarks-but-better.desktop")
        );
        assert_eq!(
