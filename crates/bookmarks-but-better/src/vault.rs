...
pub struct Vault {
    ...
    root: Dir,
    /// `<vault>/.bookmarks-but-better`, where staging lives. Absent when the vault was opened
    /// read-only, which is the offline `bookmarks-but-better rescan` path and never mutates.
    state: Option<Dir>,
...
impl Vault {
    ...
    /// reversible delete needs the `.bookmarks-but-better` directory this deliberately does not
    /// create. It is the offline `bookmarks-but-better rescan` path, which only reads.
    ///
    /// # Errors
    ///
    ...
    /// Opens `root` for reading and writing, with `state` as `<vault>/.bookmarks-but-better`.
    ///
    /// # Errors
    ///
    /// As [`Vault::open`].
    ...
    /// change something also refreshes `bookmarks_but_better_updated`.
    ///
    /// # Errors
    ///
    ...
    /// `<vault>/.bookmarks-but-better/staging` first, and only destroyed once both have moved,
...
///
/// A directory with no `.bookmarks-but-better-folder.md` has no stable identity, so nothing can
...
/// not as it was, and `bookmarks-but-better doctor` needs to be the next thing they run.
        ...
        Err(error) => Problem::new(
            ...
            format!(
                "{}, and undoing what had already been done failed as well ({error}); nothing \
                 was discarded — run `bookmarks-but-better doctor` and see .bookmarks-but-better/staging/recovery.txt",
                problem.detail()
            ),
        ),
    }
...
fn update_bookmark(
    ...
    // `bookmarks_but_better_updated` first would have made it one.
    let probe = file
...
/// Applies a title change to one `.bookmarks-but-better-folder.md`, bound to its identity.
fn update_folder(
    vault: &Vault,
    dir: &Dir,
    origin: &[String],
        ...
        crate::staging::TakeError::Orphaned(cause) => Problem::new(
            ...
            format!(
                "{subject} was moved out of the vault and could not be put back ({}); it is held \
                  in the vault's `.bookmarks-but-better/staging` directory. Recovery will try again at the next \
                  start; if the destination is occupied, use `bookmarks-but-better doctor` and see \
                  `.bookmarks-but-better/staging/recovery.txt`",
                cause.kind()
            ),
        ),
        ...
        Err(restore) => Problem::new(
            ...
            format!(
                "{context} ({}), and undoing the change failed as well ({}); the entries are held \
                 in the vault's `.bookmarks-but-better/staging` directory and are cleared on the next start",
                cause.kind(),
                restore.kind()
            ),
        ),
...
fn hash_folder(folder: &FolderNode, hasher: &mut DefaultHasher) {
    ...
    for child in folder.children() {
        match child {
            bookmarks_but_better_vault_core::ChildNode::Folder(folder) => {
                hash_folder(folder, hasher);
            }
            bookmarks_but_better_vault_core::ChildNode::Bookmark(bookmark) => {
...
fn hash_diagnostics(
    diagnostics: &[bookmarks_but_better_vault_core::Diagnostic],
