...
//! This is the same model [`bookmarks_but_better_vault_core::scan`] already walks the vault with.
//!
...
use bookmarks_but_better_vault_core::Revision;
...
///
/// This mirrors [`bookmarks_but_better_vault_core::scan`]'s refusal exactly, and for the same
...
fn create_temp(dir: &Dir, destination: &str) -> io::Result<(String, cap_std::fs::File)> {
    ...
    for attempt in 0..TEMP_NAME_ATTEMPTS {
        let candidate = format!(".{stem}.{attempt}.bookmarks-but-better-tmp");
...
///
/// `.bookmarks-but-better` is the one name [`component::check`] refuses, because no vault
/// content may resolve into it — so the daemon needs an explicit door of its
/// own rather than a hole in the check.
///
/// # Errors
///
/// Returns the underlying I/O error, including a refusal when `.bookmarks-but-better` exists but
...
mod tests {
    ...
    fn open_or_create_dir_refuses_a_symlink() {
        ...
        std::os::unix::fs::symlink(
            outer.path().join("elsewhere"),
            vault.join(".bookmarks-but-better"),
        )
        .expect("symlink");

        let root = open_root(&vault).expect("open root");
        let error = open_or_create_dir(&root, ".bookmarks-but-better")
            .expect_err("a symlinked .bookmarks-but-better is refused");
        assert!(
    ...
    fn replace_has_the_same_observable_contract_on_every_platform() {
        ...
        let leftovers: Vec<String> = std::fs::read_dir(directory.path())
            ...
            .filter(|name| name.ends_with(".bookmarks-but-better-tmp"))
            .collect();
