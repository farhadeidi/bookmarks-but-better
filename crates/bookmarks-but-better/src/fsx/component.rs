...
//! `.bookmarks-but-better` is refused as well: it is the daemon's own state directory, and no
...
pub(crate) fn check(name: &str) -> Result<(), ComponentError> {
    ...
    if bookmarks_but_better_vault_core::is_reserved_stem(name) {
...
mod tests {
    ...
    fn the_state_directory_is_refused() {
        assert_eq!(
            check(".bookmarks-but-better"),
            Err(ComponentError::ReservedState)
        );
        // Only by that exact name; a bookmark may still be called
        // `.bookmarks-but-betterr`.
        check(".bookmarks-but-betterr").expect("a different name is fine");
    }
    ...
    fn a_path_splits_into_validated_components() {
        ...
        assert_eq!(
            split("Dev/.bookmarks-but-better").expect_err("state").1,
