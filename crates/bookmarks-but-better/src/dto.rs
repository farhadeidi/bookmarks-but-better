...
use bookmarks_but_better_vault_core::{
    Access, BookmarkNode, ChildNode, Diagnostic, FolderNode, VaultScan,
};
...
pub struct BookmarkDto {
    ...
    /// `.bookmarks-but-better-folder.md` has no revision to be stale against.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub(crate) fn folder_dto(node: &FolderNode, parent: Option<EntryRef>) -> BookmarkDto {
    ...
    BookmarkDto {
        ...
        // A directory with no `.bookmarks-but-better-folder.md` is unwritable even though the
