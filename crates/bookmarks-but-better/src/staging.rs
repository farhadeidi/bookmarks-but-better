...
//!
//! * **stage** — an entry is *renamed* into `<vault>/.bookmarks-but-better/staging/<id>/` rather
...
//! * **order** — a folder's `.bookmarks-but-better-state.json`. The previous bytes are copied
...
//! `.bookmarks-but-better/staging/recovery.txt`, and reported through `GET /api/v1/health` and
//! `bookmarks-but-better doctor`. An earlier version removed the staging directory at startup,
...
use bookmarks_but_better_vault_core::{MAX_DOCUMENT_BYTES, STATE_FILE_NAME};
...
///
/// The name inside the folder is always `.bookmarks-but-better-state.json`, so it is not
...
struct Manifest {
    ...
    ///
    /// It is what lets `bookmarks-but-better doctor` tell a stuck change from one a running
    ...
    /// exist, and a person or `bookmarks-but-better doctor` decides what to do with it.
    #[serde(default)]
...
pub(crate) struct Staged {
    /// The `.bookmarks-but-better/staging/<id>` handle everything is renamed into.
    directory: Dir,
...
impl Staged {
    ...
    pub(crate) fn commit(mut self) -> io::Result<()> {
        ...
        // it leaves stays for recovery and `bookmarks-but-better doctor` to describe — the change
    ...
    /// could not be undone stays recorded, so recovery and `bookmarks-but-better doctor` can
    /// still describe it.
    ...
    pub(crate) fn summary(&self) -> String {
        format!(
            "{} {} from an interrupted {} could not be recovered ({}); they are kept in {} — see \
             .bookmarks-but-better/{STAGING_DIRECTORY}/{RECOVERY_NAME}",
            self.entries.len(),
            ...
            Err(reason) => report.push((
                ...
                format!(
                    "entries from an interrupted change are held here and {reason}, so the daemon \
                     will not act on them; see .bookmarks-but-better/{STAGING_DIRECTORY}/{RECOVERY_NAME}"
                ),
            )),
        }
    }
...
/// operation directory then survives to hold it, and `bookmarks-but-better doctor` reports it for
/// a person to discard.
///
...
/// change did not install**. A `.bookmarks-but-better-state.json` that has been rewritten since
...
/// exposure is to another writer in `.bookmarks-but-better/staging`, which is the same trust
...
fn staged_path(name: &str) -> String {
    format!(".bookmarks-but-better/{STAGING_DIRECTORY}/{name}")
}
...
mod tests {
    ...
    fn fixture() -> Fixture {
        ...
        let state = fsx::open_or_create_dir(&vault, ".bookmarks-but-better").expect("state dir");
    ...
    fn a_hostile_manifest_is_never_acted_on() {
        for (label, body) in [
            ...
            (
                ...
                r#"{"version":2,"operation":"delete_bookmark","phase":"staging","entries":[
                   {"origin":[".bookmarks-but-better"],"name":"lock","staged":"0-x","kind":"file"}]}"#,
            ),
            (
    ...
    fn a_rollback_takes_a_created_entry_out_of_the_vault_and_keeps_it() {
        ...
        // entry is reported through `bookmarks-but-better doctor`, not through this return value.
        staged.rollback().expect("rollback");
    ...
    /// have been put there by anyone. `.bookmarks-but-better-folder.md` is not proof of authorship
    ...
    fn a_rollback_quarantines_a_created_directory_with_everything_in_it() {
        ...
        fsx::create_new(&child, ".bookmarks-but-better-folder.md", b"NOT OURS").expect("metadata");
        drop(child);
        ...
        assert_eq!(
            fsx::read(&kept, ".bookmarks-but-better-folder.md").expect("read"),
            b"NOT OURS",
    ...
    /// through, and the kept directory would carry no mark for `bookmarks-but-better doctor`.
    #[test]
    ...
    fn an_unaccounted_entry_in_staging_is_reported_rather_than_deleted() {
        ...
        // `bookmarks-but-better doctor` never mentions the directory it just kept.
        assert!(
    ...
    fn a_manifest_with_a_hostile_action_is_never_acted_on() {
        for (label, body) in [
            ...
            (
                ...
                r#"{"version":4,"operation":"set_order","phase":"staging","actions":[
                   {"action":"order","origin":[".bookmarks-but-better"],"backup":"state-0.json",
                    "installed":{"revision":"00","identity":{"unavailable":null}}}]}"#,
            ),
        ] {
            let fixture = fixture();
