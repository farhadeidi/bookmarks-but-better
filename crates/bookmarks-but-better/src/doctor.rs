//! The read-only health report behind `bookmarks-but-better doctor`.
//!
...
//!
//! It also looks in `.bookmarks-but-better/staging`, which no scan covers: entries a crashed run
...
use bookmarks_but_better_vault_core::{FolderNode, Severity, StateAccess, VaultScan, scan};
...
pub struct Report {
    ...
    /// Whether the root has a usable `.bookmarks-but-better-folder.md`.
...
fn collect_unorderable(folder: &FolderNode, out: &mut Vec<Finding>) {
    if folder.state_access() == StateAccess::ReadOnly {
        out.push(Finding {
            ...
            detail: "this folder's `.bookmarks-but-better-state.json` holds something this build must not \
...
fn count_folders(folder: &bookmarks_but_better_vault_core::FolderNode) -> usize {
    folder
        .folders()
...
/// Entries in `.bookmarks-but-better/staging` that need a person.
///
...
fn daemon_is_running(root: &Path) -> bool {
    let Ok(handle) = std::fs::File::options()
        .read(true)
        .write(true)
        .open(root.join(".bookmarks-but-better").join("lock"))
    else {
        return false;
    };
...
mod tests {
    use super::*;
    use bookmarks_but_better_vault_core::{FOLDER_FILE_NAME, STATE_FILE_NAME};
    ...
    fn a_child_order_file_that_cannot_be_rewritten_is_named() {
        ...
        std::fs::write(
            dir.path()
                .join("Dev")
                .join(bookmarks_but_better_vault_core::STATE_FILE_NAME),
            "{ not json",
        )
        .expect("order file");
    ...
    fn every_way_of_reporting_a_frozen_order_agrees() {
        ...
        let scanned = bookmarks_but_better_vault_core::scan(dir.path()).expect("scan");
