...
use bookmarks_but_better_vault_core::STATE_FILE_NAME;
use serde_json::json;
...
async fn an_externally_added_bookmark_is_appended_and_adopted_by_the_next_change() {
    ...
    write_external(
        harness.root(),
        "Outside--88888888.md",
        "---\nbookmarks_but_better_id: 88888888\nbookmarks_but_better_url: https://outside.example\nbookmarks_but_better_title: Outside\n\
         bookmarks_but_better_created: 2020-01-01T00:00:00Z\nbookmarks_but_better_updated: 2020-01-01T00:00:00Z\n---\n",
    );
    harness.post("/api/v1/rescan", &json!({})).await;
...
async fn a_reference_to_something_that_is_not_there_is_kept_across_a_reorder() {
    ...
    write_external(
        harness.root(),
        &format!("Two--{two}.md"),
        "---\nbookmarks_but_better_id: 00000000\n---\n",
    );
    let restored = format!(
        "---\nbookmarks_but_better_id: {two}\nbookmarks_but_better_url: https://two.example\nbookmarks_but_better_title: Two\n\
...
async fn a_cloud_client_that_delivers_half_a_change_is_absorbed() {
    ...
    write_external(
        harness.root(),
        "Late--77777777.md",
        "---\nbookmarks_but_better_id: 77777777\nbookmarks_but_better_url: https://late.example\nbookmarks_but_better_title: Late\n\
         bookmarks_but_better_created: 2026-01-01T00:00:00Z\nbookmarks_but_better_updated: 2026-01-01T00:00:00Z\n---\n",
    );
    harness.post("/api/v1/rescan", &json!({})).await;
    let children = harness.child_ids(&root).await;
...
async fn an_index_means_the_visual_position_and_a_position_it_cannot_record_is_refused() {
    ...
    assert!(
        detail.contains("no `.bookmarks-but-better-folder.md`"),
...
async fn a_directory_with_no_identity_cannot_be_ordered_and_never_blocks_a_reorder() {
    let harness = Harness::new();
    let (root, [one, two, dev, ops]) = populated(&harness).await;
    // A folder the user made in a file manager: no `.bookmarks-but-better-folder.md`, so nothing
