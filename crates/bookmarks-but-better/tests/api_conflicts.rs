...
async fn a_stale_revision_is_a_conflict_and_never_an_overwrite() {
    ...
    let external = std::fs::read_to_string(&path).expect("read").replace(
        "bookmarks_but_better_title: React",
        "bookmarks_but_better_title: React (theirs)",
    );
...
async fn two_entries_claiming_one_identity_are_ambiguous() {
    let harness = Harness::new();
    let body = "---\nbookmarks_but_better_id: eeeeffff\nbookmarks_but_better_url: https://example.com\nbookmarks_but_better_title: Twin\n\
                bookmarks_but_better_created: 2026-01-01T00:00:00Z\nbookmarks_but_better_updated: 2026-01-01T00:00:00Z\n---\n";
