...
use bookmarks_but_better_vault_core::FOLDER_FILE_NAME;
use serde_json::json;
...
async fn a_created_bookmark_round_trips_through_markdown() {
    ...
    assert!(
        text.contains("bookmarks_but_better_url: https://react.dev"),
        "{text}"
    );
    assert!(text.contains("bookmarks_but_better_title: React"), "{text}");
...
async fn a_title_edit_keeps_the_id_and_every_unknown_byte() {
    ...
    assert!(
        after.contains("bookmarks_but_better_title: React 19"),
        "{after}"
    );
    assert!(
        after.contains(&format!("bookmarks_but_better_id: {id}")),
