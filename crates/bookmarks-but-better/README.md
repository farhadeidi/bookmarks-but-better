# `bookmarks-but-better` — the daemon, HTTP API and CLI
...
[`bookmarks-but-better-vault-core`](../bookmarks-but-better-vault-core); this crate adds behaviour — locking,
...
`bookmarks-but-better service install` writes a **user-level** definition — no administrator, no
...
failure — `bookmarks-but-better service status` says so rather than implying otherwise.
...
- **Restart on failure**, where the platform supports it — never on a clean
  exit, so `bookmarks-but-better service stop` is a command rather than a fight.
...
`bookmarks-but-better service start`/`stop`/`status` to drive. `bookmarks-but-better service install` says so
...
```jsonc
{
  ...
  "dateAdded": 1767225600000, // epoch milliseconds, parsed from bookmarks_but_better_created
...
| `subtree_has_unknown_files` | 409 | the folder holds files bookmarks-but-better does not manage |
...
| `state_read_only`   | 422    | the folder's `.bookmarks-but-better-state.json` must not be overwritten |
...
Each managed folder owns a `.bookmarks-but-better-state.json` recording the order its children
...
by `bookmarks_but_better_created` and stable identity, and finally any directory with no
`.bookmarks-but-better-folder.md`. It gains a real order file the first time a change needs one,
...
achieve it, and `bookmarks-but-better init` on an existing vault pins whatever order is being
shown at that moment.

A directory with no `.bookmarks-but-better-folder.md` has no stable identity, so no order file
...
deleting all keep working. `bookmarks-but-better doctor` names every folder in that state.

### Deleting a folder

`recursive=true` deletes a subtree only when every file in it is one the vault
manages — a `.bookmarks-but-better-folder.md`, a scanned bookmark, or a file inside a scanned
bookmark's `.assets` directory, a nested `.bookmarks-but-better-folder.md` or a nested
`.bookmarks-but-better-state.json` — and every one of those files still holds the bytes the
...
- **Nothing is deleted in place.** Deletions rename entries into
  `<vault>/.bookmarks-but-better/staging` under a durable, two-phase manifest, and destroy them
  ...
  be recovered is kept, listed in `.bookmarks-but-better/staging/recovery.txt`, and reported in
  `GET /health` and `bookmarks-but-better doctor`.
...
- **An undo that fails never deletes anything.** The temporary then holds the
  ...
  destination is occupied, the bytes remain staged and `bookmarks-but-better doctor` explains
  how to recover them.
...
- **Nothing outside the vault.** Every read, write and rename resolves through
  ...
  redirecting it. `.bookmarks-but-better` is refused if it is not a real directory. The static
...
Every guarantee above is about `bookmarks-but-better` itself, and each one still holds: the
...
make `bookmarks-but-better` talk to non-loopback hosts for the first time and needs the SSRF
...
```sh
cargo test -p bookmarks-but-better
# the cfg(windows) code, checked from any host:
rustup target add x86_64-pc-windows-msvc
cargo check --workspace --all-targets --target x86_64-pc-windows-msvc
```

`bookmarks-but-better doctor` reports a staging directory only when recovery marked it retained
