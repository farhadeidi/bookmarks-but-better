...
fn a_title_change_touches_exactly_one_line() {
    for case in VALID {
        ...
        assert!(
            changed[0].1.contains("bookmarks_but_better_title"),
...
fn quoting_style_is_preserved_and_upgraded_only_when_needed() {
    ...
    assert!(
        text.contains("bookmarks_but_better_url: 'https://example.com/plain'"),
        "{text}"
    );
    assert!(
        text.contains("bookmarks_but_better_title: \"still double quoted\""),
        "{text}"
    );
}
...
fn a_plain_field_is_quoted_when_the_new_value_needs_it() {
    ...
    let text = String::from_utf8(written).expect("valid UTF-8");
    assert!(
        text.contains("bookmarks_but_better_title: \"true\""),
        "{text}"
    );
...
fn an_absent_optional_key_is_inserted_next_to_the_owned_keys() {
    ...
    assert!(
        text.contains(
            "bookmarks_but_better_updated: 2026-01-02T10:30:00Z\nbookmarks_but_better_logo: React--a1b2c3d4.assets/logo.png\n---"
        ),
        "{text}"
    );
...
fn an_empty_optional_key_is_filled_in_place_and_keeps_its_comment() {
    ...
    let text = String::from_utf8(written).expect("valid UTF-8");
    assert!(
        text.contains("bookmarks_but_better_logo: logo.png # filled in later\n"),
        "{text}"
    );
...
fn clearing_an_optional_key_removes_its_whole_line() {
    ...
    let text = String::from_utf8(written).expect("valid UTF-8");
    assert!(!text.contains("bookmarks_but_better_logo"), "{text}");
    assert!(
        text.contains("bookmarks_but_better_title: Has a logo\nbookmarks_but_better_created:"),
        "{text}"
    );
...
fn values_that_cannot_round_trip_are_refused() {
    ...
    let cases = [
        (
            ...
            "bookmarks_but_better_title",
        ),
        (BookmarkUpdate::new().url("   "), "bookmarks_but_better_url"),
        (
            BookmarkUpdate::new().created("yesterday"),
            "bookmarks_but_better_created",
        ),
        (
            BookmarkUpdate::new().updated("2026-13-01T00:00:00Z"),
            "bookmarks_but_better_updated",
        ),
        (BookmarkUpdate::new().logo(""), "bookmarks_but_better_logo"),
    ];
...
fn a_degraded_document_stays_writable_and_explains_itself() {
    ...
    assert!(
        String::from_utf8_lossy(&written)
            .contains("bookmarks_but_better_extra: reserved but unknown")
    );
}
...
fn a_read_only_document_cannot_be_written() {
    // `bookmarks_but_better_url` is empty, which is an error-level diagnostic.
    let source = b"---\nbookmarks_but_better_id: a1b2c3d4\nbookmarks_but_better_url:\nbookmarks_but_better_title: No URL\n\
                   bookmarks_but_better_created: 2026-01-01T00:00:00Z\nbookmarks_but_better_updated: 2026-01-01T00:00:00Z\n---\n";
...
fn folder_metadata_round_trips() {
    ...
    assert_eq!(
        ...
        "---\nbookmarks_but_better_id: 3a4b5c6d\nbookmarks_but_better_title: Inbox\n---\n"
    );
...
fn folder_metadata_preserves_unknown_keys_and_crlf() {
    ...
    let text = String::from_utf8(written).expect("valid UTF-8");
    assert!(
        text.contains("bookmarks_but_better_title: Books\r\n"),
        "{text}"
    );
...
fn folder_metadata_without_an_identity_is_an_error() {
    let error = FolderFile::parse(b"---\nbookmarks_but_better_title: No identity\n---\n")
...
fn rendered_documents_are_parsable_and_canonical() {
    ...
    let rendered = bookmarks_but_better_vault_core::render_bookmark(
        id,
        "https://example.com",
        "New bookmark",
    ...
    assert_eq!(
        rendered,
        "---\nbookmarks_but_better_id: a1b2c3d4\nbookmarks_but_better_url: https://example.com\nbookmarks_but_better_title: New bookmark\n\
         bookmarks_but_better_created: 2026-01-01T00:00:00Z\nbookmarks_but_better_updated: 2026-01-01T00:00:00Z\n---\n"
    );
    ...
    assert_eq!(
        bookmarks_but_better_vault_core::render_folder(id, Some("Inbox")).unwrap(),
        "---\nbookmarks_but_better_id: a1b2c3d4\nbookmarks_but_better_title: Inbox\n---\n"
    );
    assert_eq!(
        bookmarks_but_better_vault_core::render_folder(id, None).unwrap(),
        "---\nbookmarks_but_better_id: a1b2c3d4\n---\n"
    );
}
