//! How a folder's `.bookmarks-but-better-state.json` decides the order its children come back in.
//!
...
fn vault() -> TempDir {
    ...
    dir.write(
        "Dev/.bookmarks-but-better-folder.md",
        folder_source("dddddddd", Some("Dev")),
    );
    dir.write(
        "Ops/.bookmarks-but-better-folder.md",
        folder_source("eeeeeeee", Some("Ops")),
    );
    dir
}
...
fn diagnostic_codes(scan: &VaultScan) -> Vec<DiagnosticCode> {
    scan.diagnostics()
        .into_iter()
        .map(bookmarks_but_better_vault_core::Diagnostic::code)
        .collect()
}
...
fn bookmarks_without_a_recorded_order_sort_by_creation_then_identity() {
    ...
    dir.write(
        "Broken--qqqqqqqq.md",
        "---\nbookmarks_but_better_id: qqqqqqqq\nbookmarks_but_better_url: https://q.example\nbookmarks_but_better_title: Broken\n\
         bookmarks_but_better_created: yesterday\nbookmarks_but_better_updated: yesterday\n---\n",
    );
...
/// A directory with no `.bookmarks-but-better-folder.md` cannot be named by any order file, so
...
fn a_sibling_that_differs_only_by_case_freezes_the_order() {
    let dir = vault();
    dir.write(STATE_FILE_NAME, state(&[("bbbbbbbb", ChildKind::Bookmark)]));
    dir.write(".BOOKMARKS-BUT-BETTER-State.JSON", "{}");
