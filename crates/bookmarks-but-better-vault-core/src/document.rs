...
pub const KEY_ID: &str = "bookmarks_but_better_id";
/// The front matter key holding a bookmark's target URL.
pub const KEY_URL: &str = "bookmarks_but_better_url";
/// The front matter key holding a display title.
pub const KEY_TITLE: &str = "bookmarks_but_better_title";
/// The front matter key holding the creation timestamp.
pub const KEY_CREATED: &str = "bookmarks_but_better_created";
/// The front matter key holding the modification timestamp.
pub const KEY_UPDATED: &str = "bookmarks_but_better_updated";
/// The front matter key holding a vault-relative logo path.
pub const KEY_LOGO: &str = "bookmarks_but_better_logo";

/// The namespace this crate owns. Any other key is user data.
const OWNED_PREFIX: &str = "bookmarks_but_better_";
...
pub enum ParseError {
    ...
    /// `bookmarks_but_better_id` is present but is not a valid identity.
    InvalidId {
        /// The 1-based line of `bookmarks_but_better_id`.
        line: usize,
        /// Why the identity is invalid.
        reason: IdError,
    },
    /// The file carries no `bookmarks_but_better_id`, so it is ordinary Markdown, not a bookmark.
    NotManaged,
}
...
fn scan_owned(
    ...
    // after the owned-key pass so that a duplicated `bookmarks_but_better_*` key is reported by
...
///
/// `bookmarks_but_better_id` is deliberately absent: identity is immutable.
...
impl BookmarkUpdate {
    ...
    /// Sets `bookmarks_but_better_title`.
    #[must_use]
    ...
    /// Sets `bookmarks_but_better_created`.
    #[must_use]
    ...
    /// Sets `bookmarks_but_better_updated`.
    #[must_use]
    ...
    /// Sets `bookmarks_but_better_logo`, inserting the key when it is absent.
    #[must_use]
    ...
    /// Removes `bookmarks_but_better_logo` and its whole line.
    #[must_use]
...
/// A parsed `.bookmarks-but-better-folder.md` metadata file.
#[derive(Debug, Clone)]
...
impl FolderFile {
    ...
    /// # Errors
    ///
    /// As [`BookmarkFile::parse`]. A folder file without `bookmarks_but_better_id` is an error
    ...
    /// Rewrites `bookmarks_but_better_title`, leaving every other byte alone.
    ///
    /// # Errors
    ///
...
impl FolderUpdate {
    ...
    /// Sets `bookmarks_but_better_title`, inserting the key when it is absent.
    #[must_use]
    pub fn title(mut self, value: impl Into<String>) -> Self {
    ...
    /// Removes `bookmarks_but_better_title` and its whole line.
    #[must_use]
...
/// Renders a minimal, canonical `.bookmarks-but-better-folder.md` with LF line endings.
///
/// # Errors
///
