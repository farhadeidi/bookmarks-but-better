...
//!   `.bookmarks-but-better-folder.md` file holding the directory's stable identity, and a
//!   `.bookmarks-but-better-state.json` recording the order its children are in.
...
//!
//! Owned front matter keys are namespaced with a `bookmarks_but_better_` prefix:
//!
//! | key                              | bookmark | folder   | meaning                    |
//! |----------------------------------|----------|----------|----------------------------|
//! | `bookmarks_but_better_id`        | required | required | stable identity            |
//! | `bookmarks_but_better_url`       | required | –        | target URL                 |
//! | `bookmarks_but_better_title`     | required | optional | display title              |
//! | `bookmarks_but_better_created`   | required | –        | RFC 3339 creation time     |
//! | `bookmarks_but_better_updated`   | required | –        | RFC 3339 modification time |
//! | `bookmarks_but_better_logo`      | optional | –        | vault-relative logo path   |
//!
...
//! assert!(String::from_utf8_lossy(&renamed).contains("bookmarks_but_better_title: Renamed\r\n"));
