...
//!
//! * the order recorded in the folder's `.bookmarks-but-better-state.json`, for every child that
//!   file names and the directory actually holds; then
//! * every remaining child, in the deterministic *migration order* — folders
//!   first, then by `bookmarks_but_better_created` and stable identity.
//!
...
pub const FOLDER_FILE_NAME: &str = ".bookmarks-but-better-folder.md";
...
impl BookmarkNode {
    ...
    /// The display title: `bookmarks_but_better_title`, or the readable part of the filename.
    #[must_use]
...
pub enum StateAccess {
    /// The folder has no `.bookmarks-but-better-state.json` yet, so one is written the first
...
impl FolderNode {
    ...
    /// The display title: `bookmarks_but_better_title`, or the directory name.
    #[must_use]
    pub fn title(&self) -> &str {
        &self.title
    }

    /// The content revision of `.bookmarks-but-better-folder.md`, when it exists.
    #[must_use]
    ...
    /// usable `.bookmarks-but-better-state.json`, and the deterministic migration order where it
    /// does not. See [`scan`].
    #[must_use]
    ...
    /// The content revision of `.bookmarks-but-better-state.json`, when it exists and is
    /// readable.
    ///
...
/// The `.bookmarks-but-better-state.json` a directory holds, however usable it turned out to be.
#[derive(Debug)]
struct StateFile {
...
fn migration_key(child: &ChildNode) -> (u8, u8, bool, String, String, String) {
    ...
    (
        // A directory with no `.bookmarks-but-better-folder.md` has no identity, so no order
...
/// Interprets the bytes of one `.bookmarks-but-better-state.json`.
...
fn visit_file(
    ...
    match BookmarkFile::parse_at(&bytes, Some(&child_relative)) {
        ...
        // no `bookmarks_but_better_id`, is the user's own content and is passed over in silence.
