...
with no command, it installs the daemon when none is installed and reports
status otherwise.
...
Commands:
  ...
  vault add <id> <path>  Configure another vault and restart the service.
...
export function parseArgs(argv) {
  ...
  if (command !== null && !COMMANDS.includes(command)) {
    ...
    } else if (subcommand === "add" && args.length !== 2) {
      errors.push("vault add needs exactly an id and a path");
    } else if (subcommand === "remove" && args.length !== 1) {
      errors.push("vault remove needs exactly an id");
